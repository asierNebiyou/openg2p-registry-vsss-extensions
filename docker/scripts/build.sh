...
# workflows for the Village Social Security System (VSSS) repo.
#
...
if [[ "${BUILD_PLATFORM}" == *","* ]]; then
  ...
  if ! docker buildx inspect openg2p-vsss-builder &>/dev/null; then
    docker buildx create --name openg2p-vsss-builder --use
  else
    docker buildx use openg2p-vsss-builder
  fi
...
for SERVICE_FILE in "${SERVICE_FILES[@]}"; do
  ...
  # --source-root is the project root — it's where ./openg2p-registry-vsss-extension and other
  ...
  BUILD_ARGS=(
    ...
    --label "org.opencontainers.image.description=OpenG2P Village Social Security System service image"
  )
