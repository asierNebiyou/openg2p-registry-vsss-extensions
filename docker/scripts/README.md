# docker/scripts/ — Local build scripts for the VSSS Docker images
...
```bash
cd /path/to/national-social-registry
...
```bash
docker inspect --format '{{.Architecture}}' openg2p/openg2p-vsss-staff-portal-api:develop
# → amd64
```
...
./openg2p-registry-vsss-extension                                               ← local-path dep (relative to project root)
...
For **local-path** entries (e.g. `./openg2p-registry-vsss-extension`), `parse_service.py`
...
When called with no arguments, `build.sh` builds all three backend services:

| Service | Spec File (relative to `docker/`) | Produces |
|---------|-----------|----------|
| staff-portal-api | `staff-portal-api/develop.txt` | `openg2p/openg2p-vsss-staff-portal-api:develop` |
| celery | `celery/develop.txt` | `openg2p/openg2p-vsss-celery:develop` |
| partner-api | `partner-api/develop.txt` | `openg2p/openg2p-vsss-partner-api:develop` |
...
3. Update any git-branch references if pinning to a specific VSSS extension tag
